...
const svg = {
  ...
  fullscreen: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;"><path d="M8 3H5a2 2 0 00-2 2v3m18 0V5a2 2 0 00-2-2h-3m0 18h3a2 2 0 002-2v-3M3 16v3a2 2 0 002 2h3"/></svg>'
};

html = html.replace(/🍽️ Order Now/, svg.utensils + ' Order Now');
html = html.replace(/<span class="sound-icon">🔇<\/span>/, '<span class="sound-icon">' + svg.mute + '</span>');
html = html.replace(/<span class="theme-icon">☀️<\/span>/, '<span class="theme-icon">' + svg.sun + '</span>');
html = html.replace(/❤️/g, svg.heartOutline); // All hearts in gallery and lightbox
html = html.replace(/⛶/g, svg.fullscreen); // Pip and fullscreen buttons

fs.writeFileSync(path, html, 'utf8');
console.log('✅ Final emojis fixed in HTML');
